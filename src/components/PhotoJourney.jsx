...
export default function PhotoJourney() {
  ...
  // Bild blendet jetzt erst sehr spät aus: volle Sichtbarkeit bis ~75%, Fade zwischen 75% und 82%
  const imageOpacity = useTransform(scrollYProgress, [0, 0.75, 0.82], [1, 1, 0])

  // Intro bleibt im DOM bis etwas weiter gescrolled wurde (für smoother Transition)
  const [mountIntro, setMountIntro] = useState(true)
  const initialProgress = useRef(null)
  // Elegante Transformations-Kurven
  ...
  // Eintrittsanimationen entfernt -> statisch

  // Entfernt: IntersectionObserver für Story – nicht mehr benötigt

  // Ausblenden erst nach tatsächlichem Scroll: warte auf Differenz gegenüber initialem Wert
  useEffect(() => {
    const unsubscribe = scrollYProgress.on('change', v => {
      // Unmount erst später (Performance & keine Fokus-Sprünge beim schnellen Hochscrollen)
  ...
  {/* Entfernt: Heller Verlauf links */}
      </motion.div>

      {/* Fixierter Textlayer (fade in/out; kehrt beim Hochscrollen zurück) */}
      {mountIntro && (
      <motion.div
        className="fixed inset-0 z-0 flex items-center pointer-events-none"
